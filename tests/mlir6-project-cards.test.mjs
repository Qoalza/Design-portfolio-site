...
test("project tags use the exact Source Code Pro technology style", () => {
  for (const key of ["main", "home", "projects", "pageHeader"]) {
    assert.match(sources[key], /font-family:\s*var\(--type-tech-font-family\)/);
    assert.match(sources[key], /font-size:\s*var\(--type-tech-m-font-size\)/);
    assert.match(sources[key], /font-feature-settings:\s*var\(--type-tech-m-features\)/);
    assert.match(sources[key], /letter-spacing:\s*var\(--type-tech-m-letter-spacing\)/);
  }
});

test("card Update info is a 40px padded Source Code Pro frame", () => {
  for (const key of ["main", "home", "projects"]) {
    assert.match(sources[key], /\.updated\s*\{[^}]*height:\s*40px;[^}]*padding:\s*0 16px;[^}]*font-family:\s*var\(--type-tech-font-family\)[^}]*font-feature-settings:\s*var\(--type-tech-m-features\)/s);
  }
});
