...
test("frame import snapshots each direct visual unit as a separate 2x raster and preserves root constraints and effects", async () => {
  ...
  assert.equal(manifest.nodes[2].asset.fit, "contain", "a ready PNG must remain fully visible instead of being cropped by cover");
  assert.equal(manifest.nodes[2].stroke, undefined, "a ready image keeps its baked border and must not receive a second CSS stroke");
  ...
  assert.equal(manifest.nodes[2].asset.bounds, undefined, "the original image fill must not be expanded or cropped using rendered export bounds");
