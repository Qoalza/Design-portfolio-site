...
| User contract | Scroll mechanics | Downward entry inertia stops at the exact sticky start with progress 0; after 120 ms without virtual-scroll events the next independent gesture starts the tape without losing its first event; reverse entry remains continuous | 1615 px horizontal / 9690 px vertical (6 vertical px per horizontal px); an internal subscription uses the existing Lenis instance, with one RAF, no native wheel listener and unchanged global `wheelMultiplier: 1` | strong entry gesture stopped at offset 0/progress 0; the next gesture advanced to 0.1567 and reverse reduced it to 0.0294 |
