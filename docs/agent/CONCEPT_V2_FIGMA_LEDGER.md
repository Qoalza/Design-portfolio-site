...
| `3110:65162`, `3110:65196` | Shade | 638×328 Preview shade; exact radial-gradient vector; visible in Enable | transparent | Exact isolated node vector `public/figma/project-shade-preview.svg` fills only the Preview from y=1 to y=329, stays between back/front artwork, and transitions opacity in 150 ms Ease In |
...
| `3075:60331`, `3116:67190`, `3116:73083` | Outer fields | 1440×240 source frames; outer top field has a 1 px bottom stroke and outer bottom field a 1 px top stroke; the centered 1280×238 patterned child has 1 px left/right 16 px dash / 16 px gap boundaries; all use `#2e3133`. The pattern is assembled from 16×16 instances | Native HTML/CSS: full-viewport outer field and horizontal border; centered responsive `min(1280px,100%)` child with exact 1 px pseudo-element boundaries using a 16 px on / 16 px off vertical repeat and a 16 px SVG dot tile. The child follows and is clipped by the responsive field height. Per the approved short-viewport exception, both fields are visually hidden when their computed height is below 48 px, without changing the central layout geometry. No raster field asset, scaling halo or fixed-width horizontal boundary | matches; Figma outer/child context and runtime CSS guard |
...
| `3116:67342`, `3108:64566`, `3108:64578` | Tape composition | 1440×746; top 24; timeline window 1280×530; progress gap 108; bottom 80 | The moving tape is clipped by a centered `min(1280px,100%)` window; variables adapt in the explicit order 24→0, 108→24, 80→24 after free space | matches; exact context plus runtime geometry at 2560×1440, 1600, 1280 and 720 heights |
| `3108:64584`, children `3108:64585`…`3108:64592` | Current date marker | Date row 44 px; rail 32×44, self-stretch, vertical padding 4, gap 8; top and bottom node frames 4×4 containing exact 5×5 exports; center line 1×12; top stroke `#44494d`, line `#363a3d`, bottom fill/stroke `#2ed400`; date colors `#676e73/#adb3b8` | Three explicit child layers use exact downloaded SVG bytes and the same geometry; no CSS-drawn substitute | matches; exact-node context plus computed runtime geometry and asset hrefs at progress 0 |
...
| `3108:64684` | Right fade | Updated source: width 240, full 530 height; `#131414` at 3.31% to transparent | Fixed at the right edge of the centered 1280 px window | matches; fresh `get_design_context` and computed runtime width |
| `3116:67337`…`3116:67340`, user clarification | Left fade | Mirrored overlay belongs to moving states only | Fixed 240 px sibling at the left edge of the 1280 px window; absent at progress 0, visible during movement, absent again at progress 1 | exact state checks at start/intermediate/end |
...
| `3116:67337`…`3116:67340` | Storyboard reached state | Every job stays selected after the line reaches it; reverse travel clears it only after the line retreats past its boundary. Reached date colors are `#676e73/#adb3b8`; its bottom marker is exact fill/stroke `#43a2ee`, while top `#363a3d` and line `#2e3133` remain neutral | Exact node-anchor stops determine the frontier, so each new reached state begins as its node crosses the window center; every earlier index remains reached | deterministic anchor boundaries and runtime center measurements |
| User contract | Scroll mechanics | Downward entry inertia stops at the exact sticky start with progress 0; after the virtual-scroll stream actually ends and its 120 ms quiet tail elapses, the next independent gesture starts the tape without losing its first event; a continuous event stream remains held, and reverse entry remains continuous. Latest adjustment makes the tape 10% faster | 2047 px first-node-to-last-node horizontal travel / `9690 ÷ 1.1` vertical travel; the existing Lenis gate uses only the 120 ms quiet-tail detector — no hard hold ceiling — with one RAF, no native wheel listener and unchanged global `wheelMultiplier: 1` | first and last nodes measured at exact window center; anchor stop tests; entry/re-entry behavior retained |
...
## About — correction stage

| Figma node | Property | Current Figma value / binding | Implementation | Status / verification |
|---|---|---|---|---|
| `3223:139418` | Section composition | 1440×961 block: 312 px heading followed by a 649 px body; 1280 px centered content area | Native HTML/CSS heading, side hatching and body; it is inserted after Experience and before the existing footer | matches; runtime geometry at 1440×960 |
| `3214:124474` | Copy column | 684 px wide copy column, 1 px 16/16 dashed divider, 56 px horizontal padding | Separate non-clipping copy article and CSS-generated 16/16 horizontal/vertical dashes; no browser `dashed` approximation | matches; source mapping and runtime screenshot |
| `3215:124481`, `3216:124500` | Card panel | 593×649 panel; 16 px CSS point field covers the complete panel to its bottom edge | One native radial-gradient tile, `16px 16px`, scoped to the full carousel panel | matches; runtime screenshot and computed background size |
| `3223:155421` | Resting deck | front 363×420/r12; rear 217.5×296/r8; dark middle mask 401×372/blur20; active halo blur30/opacity.3 | Each real card keeps an immutable 363×420 inner content stage, uniformly scaled inside a separate clipping frame; crisp borders remain outside blurred media | matches; frame unit checks and runtime screenshot |
| `3223:155136` | Hover | fixed stack geometry; `#131414` .8 overlay + blur2; component 36 px increase action | Hover/focus only overlays the active card and exposes the real Library-compatible control; no deck re-layout | matches; DOM/visual runtime check |
| `3223:139419`, `3223:155007` | Enlarged viewer | 1440×960 centered composition; 520×602 foreground, own rear cards, blur/halo, separate 16/24 caption and 36 px controls | Fixed overlay uses the selected card’s own media in foreground, halo and rear layers; below the reference viewport the entire composition scales proportionally and controls use the Small size | matches; runtime open/Esc and viewport evidence |
| User-approved motion contract | Cycle and handoff | cyclic Я → Машина → Собачки; 500 ms; parallel handoff with separated front/incoming cards and third card behind | Pure frame function governs position, outer mask, uniform content scale, role effects and z-order; commands are ignored while a transition runs | matches; deterministic frame tests and browser interaction |
