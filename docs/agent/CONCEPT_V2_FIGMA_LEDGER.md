...
| `3075:60331`, `3116:67190` | Outer fields | 1440×240 each; outer top field has a 1 px bottom stroke and outer bottom field a 1 px top stroke; the centered 1280×239 patterned child includes its 1 px left/right strokes; all use `#2e3133` | Each complete 1440×240 node is rendered once from its exact export. No CSS border or pseudo-element duplicates the strokes, so the side gutters remain clean and the horizontal boundary stays one pixel | matches; exact Plugin API geometry, export inspection and runtime at 1920×1600 |
