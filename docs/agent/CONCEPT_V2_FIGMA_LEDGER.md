...
| `3075:60331`, `3116:67190` | Outer fields | 1440×240 source frames; outer top field has a 1 px bottom stroke and outer bottom field a 1 px top stroke; the centered 1280×239 patterned child has 1 px left/right strokes; all use `#2e3133`. The pattern is assembled from 16×16 instances | Native HTML/CSS: full-viewport 240 px outer field and horizontal border; centered responsive `min(1280px,100%)` 239 px child with border-inline and a 16 px CSS radial-dot tile. No raster field asset, scaling halo or fixed-width horizontal boundary | matches; `get_design_context` for outer/child/tile nodes plus runtime geometry at 1920×1600 |
