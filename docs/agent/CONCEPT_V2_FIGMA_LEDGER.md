...
| User contract | Scroll mechanics | Downward entry inertia stops at the exact sticky start with progress 0; after 120 ms without virtual-scroll events the next independent gesture starts the tape without losing its first event; a continuous event stream cannot hold the section indefinitely; reverse entry remains continuous. Latest adjustment makes the tape 10% faster | 2047 px first-node-to-last-node horizontal travel / `9690 ÷ 1.1` vertical travel; the existing Lenis gate has a 600 ms hard hold ceiling in addition to the 120 ms quiet-tail detector, with one RAF, no native wheel listener and unchanged global `wheelMultiplier: 1` | first and last nodes measured at exact window center; anchor stop tests; entry/re-entry behavior retained |
