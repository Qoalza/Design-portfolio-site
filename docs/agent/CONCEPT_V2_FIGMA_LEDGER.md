...
| `3116:67337`…`3116:67340` | Left fade | Mirrored 109 px overlay is present while the tape exits left in `67337`–`67339`; absent in final `67340` | Fixed sibling of the moving tape; 109 px mirrored gradient fades to opacity 0 only at normalized progress 1 | exact storyboard contexts plus computed runtime styles |
...
| `3116:67337`…`3116:67340` | Storyboard reached state | Every job stays selected after the line reaches it; reverse travel clears it only after the line retreats past its boundary. Reached date colors are `#676e73/#adb3b8`; its bottom marker is exact fill/stroke `#43a2ee`, while top `#363a3d` and line `#2e3133` remain neutral | `floor(progress × 5)` determines the frontier and every index up to it receives the reached state; exact neutral/reached SVG exports switch only the bottom marker | deterministic boundaries; runtime forward at 0.4746 and reverse at 0.1202 |
| User contract | Scroll mechanics | Downward entry inertia stops at the exact sticky start with progress 0; after 120 ms without virtual-scroll events the next independent gesture starts the tape without losing its first event; a continuous event stream cannot hold the section indefinitely; reverse entry remains continuous | 1615 px horizontal / 9690 px vertical (6 vertical px per horizontal px); the existing Lenis gate has a 600 ms hard hold ceiling in addition to the 120 ms quiet-tail detector, with one RAF, no native wheel listener and unchanged global `wheelMultiplier: 1` | strong entry stopped at progress 0; continuous input released the gate and advanced; reverse reset it to idle; the same continuous-input path worked on the second entry |
