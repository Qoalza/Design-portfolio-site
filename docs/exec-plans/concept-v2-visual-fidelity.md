...
7. Sticky Header: enters and exits from the top over 150ms; uses `Border/Surface` as its lower inset line; its shell carries the active stacking context above project-card decor; desktop Experience reserves the 80px Header height and normalizes its sticky progress against the remaining viewport.

## Verification

- Focused behavior tests per group, followed by `npm run check`, `npm run check:browser`, and `git diff --check` at the reviewed HEAD. Header proof at `2ac1f5a`: `50/50` tests plus local-runtime confirmation that the Header shell is `z-index:20` above the project divider (`8`) and the Experience sticky viewport begins at `80px`.
