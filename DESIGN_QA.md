...
MLIR5 action bar, точный внутренний layout и контракт «Поделиться» остаются `OPEN` до свежего RED→GREEN evidence. `CLOSED` разрешён только после явной пользовательской приёмки.
