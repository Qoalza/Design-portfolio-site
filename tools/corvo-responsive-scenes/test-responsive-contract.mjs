...
assert.doesNotMatch(authorization, /\.authorization \{[^}]*max-width:/, 'Authorization desktop frame has no maximum width');
...
assert.match(authorization, /@media \(min-width: 1280px\)[\s\S]*?\.auth-panel \{ border-radius: 32px; \}/, 'Authorization enters Desktop at 1280px and stays fluid above the 1600px reference viewport');
...
assert.doesNotMatch(campaignsCss, /\.campaigns \{[^}]*max-width:/, 'Every signed-in desktop frame has no maximum width');
...
assert.match(campaignsCss, /@media \(min-width: 1280px\)/, 'Media campaigns enters Desktop at 1280px and stays fluid above the 1600px reference viewport');
