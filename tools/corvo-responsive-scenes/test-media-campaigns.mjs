...
const tabletCssStart = css.indexOf('@media (min-width: 600px) and (max-width: 1599px)');
const desktopCssStart = css.indexOf('@media (min-width: 1600px)');
...
assert.doesNotMatch(desktopCss, /\.metric-card \{ flex: 0 0 440px; min-width: 440px; \}/, 'Desktop metrics retain their fluid base layout instead of locking to 440px');
...
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)/, 'Tablet owns the complete 600–1599px source range');
...
assert.match(html, /max-width: 1599px/, 'Responsive table assets retain their Tablet variants through 1599px');
...
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?\.search \{[^}]*width: 100%/, 'Tablet search occupies its source second row throughout the tablet range');
...
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?\.metric-card \{[^}]*flex: 1 0 252px[^}]*min-width: 252px/, 'Tablet cards expand while there is room, then hold their Figma minimum instead of shrinking');
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?\.metric-card p \{[^}]*font-size: 24px[^}]*line-height: 28px/, 'Tablet metric typography uses the fixed Figma 24/28px type scale throughout the breakpoint');
assert.doesNotMatch(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?\.metric-card p \{[^}]*\b(?:clamp|vw)\b/, 'Tablet metric typography cannot scale with viewport width inside one breakpoint');
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?\.table-wrap > table \{[^}]*min-width: 892px/, 'Tablet table preserves the Figma minimum for all six source columns');
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?\.table-wrap > table \{[^}]*border-collapse: separate[^}]*border-spacing: 0/, 'Tablet avoids collapsed-table sticky behavior that causes Fixed cells to slide in Zen');
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?th:first-child, td:first-child \{[^}]*background: #fff[^}]*background-clip: padding-box[^}]*left: 16px[^}]*position: sticky/, 'Tablet keeps the Figma Fixed ID & Name column inside its 16px table inset');
...
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?\.col-name \{[^}]*width: 194px[^}]*\}[\s\S]*?\.col-url, \.col-reward, \.col-geo \{[^}]*width: auto[^}]*\}[\s\S]*?\.col-created \{[^}]*width: 129px[^}]*\}[\s\S]*?\.col-action \{[^}]*width: 80px/, 'Tablet preserves all six Figma columns: fixed Name, Created and Action plus three elastic middle cells');
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?th:last-child, td:last-child \{[^}]*background: #fff[^}]*border-left: 1px solid #f2f2f2[^}]*right: 16px[^}]*position: sticky/, 'Tablet Action is a fixed trailing column with a single source leading divider');
...
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?th \{[^}]*height: 40px/, 'Tablet table header follows the 40px source row');
assert.match(css, /@media \(min-width: 600px\) and \(max-width: 1599px\)[\s\S]*?td \{[^}]*height: 56px/, 'Tablet table body follows the 56px source rows');
