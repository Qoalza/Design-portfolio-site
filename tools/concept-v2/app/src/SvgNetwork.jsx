...
export function SvgNetwork({revealed=false,idle=false}){
  const id=useId();
  const edge=`${id}-edge`;
  return <svg className={`network vector-network ${revealed?'is-revealed':''}`} viewBox={`0 0 ${WIDTH} ${HEIGHT}`} aria-hidden="true">
    ...
    <g className="construction" mask={`url(#${id}-grid)`} fill="none" strokeWidth=".7" strokeDasharray="5 5">
    ...
    <g className="network-nodes" strokeWidth={STROKE} strokeLinejoin="round">
      {nodes.map(n=>{const {x,y}=position(n);const nodeSize=(revealed?60:28)*(n[4]==='diamond'?1.2:1);const shape=glyph(n[4],x,y,nodeSize);const Shape=shape.tag;const iconSize=n[2]==='launch'||n[2]==='tools'?30:34;const iconAsset=n[2]==='tools'?'tools-refined':n[2];return <g key={n[2]} data-node={n[2]} data-shape={n[4]}>
