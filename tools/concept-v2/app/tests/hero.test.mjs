...
test('Hero lower field uses a native dot treatment over the exact Bg-main surface',async()=>{
 ...
 assert.match(desktopHero,/\.hero-bottom-dots\{[^}]*--hero-wave-mask-size:110% calc\(112% \+ 50px\)[^}]*--hero-wave-mask-position:calc\(50% \+ 8px\) -58px[^}]*display:block[^}]*inset:0[^}]*background-position:0 0[^}]*mask-image:url\('\/figma\/hero-bottom-wave-mask\.svg'\)[^}]*mask-size:var\(--hero-wave-mask-size\)[^}]*mask-position:var\(--hero-wave-mask-position\)/);
