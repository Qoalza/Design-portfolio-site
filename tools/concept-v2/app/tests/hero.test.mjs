...
test('Hero lower field uses a native dot treatment over the exact Bg-main surface',async()=>{
 ...
 assert.match(desktopHero,/\.hero-bottom-dots\{[^}]*--hero-wave-mask-size:110% calc\(112% - 56px\)[^}]*--hero-wave-mask-position:50% -8px[^}]*display:block[^}]*inset:-50px -8px 0 8px[^}]*background-position:-8px 50px[^}]*mask-image:url\('\/figma\/hero-bottom-wave-mask\.svg'\)[^}]*mask-size:var\(--hero-wave-mask-size\)[^}]*mask-position:var\(--hero-wave-mask-position\)/);
