...
export function humanError(error) {
  ...
  if (/first image dimensions/i.test(source)) return {
    title: "Размер не совпадает с первым изображением",
    message: "Первое изображение фиксирует точные ширину и высоту для этого пула. Загрузите файл того же размера или удалите все изображения из пула, чтобы задать новый размер.",
    status: 400,
  };
