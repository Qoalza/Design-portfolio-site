...
function issueFrom(error, draft) {
  ...
  if (/first image dimensions/i.test(message)) return fieldIssue({ field: location ?? "visuals", label: "Изображение", title: "Размер не совпадает с первым изображением", tab: location?.startsWith("content") ? "page" : "card", message: "Все изображения одного устройства должны иметь точные ширину и высоту первого изображения в этом пуле." });
  if (/outside the allowed pixel range/i.test(message)) return fieldIssue({ field: location ?? "visuals", label: "Изображение", title: "Размер изображения вне допустимого диапазона", tab: location?.startsWith("content") ? "page" : "card", message: "Ширина или высота файла выходит за разрешённый для устройства диапазон." });
  if (/incompatible proportion/i.test(message)) return fieldIssue({ field: location ?? "visuals", label: "Изображение", title: "Неверные пропорции изображения", tab: location?.startsWith("content") ? "page" : "card", message: "Файл не сохранён: его пропорции не соответствуют утверждённому визуальному слоту." });
