...
function issueFrom(error, draft) {
  ...
  if (/first image proportion/i.test(message)) return fieldIssue({ field: location ?? "visuals", label: "Изображение", title: "Пропорции не совпадают с первым изображением", tab: location?.startsWith("content") ? "page" : "card", message: "Все изображения одного устройства должны совпадать с первым изображением по пропорции." });
