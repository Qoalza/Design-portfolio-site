...
Admin редактирует контент проекта и принимает целый Figma Frame для карточки и hero. У такого Frame фон задаёт пользователь, внутри может быть любое число растровых элементов, constraints задают responsive-поведение, а поддержанные тени воспроизводятся Portfolio через CSS. Визуал не раскладывается по фиксированному числу скрытых слотов.
...
- Для карточки и hero Admin принимает одну ссылку на Figma Frame и импортирует Frame целиком: его фон, произвольное число растровых элементов, их координаты, constraints и тени. Для интерактивных блоков секций остаются code-owned templates.
- У закрытой страницы поле hero Frame доступно сразу. Только после успешного атомарного импорта Admin сохраняет локальный `heroFrame` и включает страницу. До этого preview и публикация остаются недоступны.
...
- Карточка и hero показывают одно поле ссылки на целый адаптивный Figma Frame; ограничений на число растровых элементов нет.
- После успешного импорта Admin показывает preview целого исходного Frame. Preview подтверждает source; фактический runtime использует сохранённые фон, геометрию, constraints и CSS-тени композиции.
...
- Schema v3 опционально хранит `catalogFrame` и `heroFrame`. Figma URL и техническое preview дополнительно сохраняются в Admin-only metadata; публичная композиция содержит только проверенные локальные raster assets, геометрию, constraints и поддержанные CSS-эффекты.
...
После сбоя на или после Commit кнопка «Продолжить» вызывает `POST /api/publish/resume` с exact `jobId`. Атомарная worker lease гарантирует один локальный worker; heartbeat обновляется каждые 5 секунд, а потерянный worker становится `ORPHANED_WORKER` не позднее 30 секунд. Worker проверяет сохранённые branch и `contentCommit`, пропускает завершённые stages и не повторяет успешно завершённые install/lint/build для того же exact SHA. Отсутствующая remote branch получает тот же SHA, совпадающая переиспользуется, а другой SHA блокирует процесс. Один открытый PR переиспользуется; закрытый неслитый или неоднозначный набор блокирует продолжение. Merge повторно читает exact PR после любого исхода команды и не удаляет ветку внутри `gh pr merge`. Непосредственно перед Deploy worker повторно читает fresh `origin/main`, доказывает, что content commit, merge SHA и текущий production являются его предками, и собирает именно fresh main; устаревший или divergent target блокируется. Компактный standalone artifact загружается через `upload-v2`, а `start-v2` запускает автономную VPS-операцию. Сохранённый `serverOperationId` позволяет после отключения Mac/VPN или перезапуска Admin продолжить polling той же операции без повторной загрузки. После публичной проверки snapshot берётся из проверенного checkout; изменённый во время публикации live draft остаётся без изменений.
...
- Schema v2 читается отдельным migrator. Существующие schema-v3 документы с `{ templateId, assets }` продолжают работать как fallback наряду с `catalogFrame` и `heroFrame`.
