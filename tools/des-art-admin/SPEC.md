...
- Галерея одна. Admin управляет только устройством и изображениями (`add/remove/reorder`); рамка, иконка, подпись и размеры принадлежат `gallery.devices-v1`. Для каждого устройства разрешён безопасный диапазон пропорций вместо одного exact ratio; Portfolio вписывает изображение через `contain` в фиксированную рамку без обрезки и изменения лейаута.
...
- Слишком большой, слишком маленький, неправильной пропорции или MIME asset отклоняется до записи; правильный сохраняется без изменения visual rules.
