...
- Runtime checkpoint: `28e5beb` (`fix(concept-v2): halve large hero dot fade`).
...
- Large runtime inspection at `2400×1280`, DPR 1: the field is `560px` high, its grid remains `16px` at `0 0`, the obsolete Large SVG mask computes to `none`, and the CSS overlay resolves to a `48% × 37.5%` radial bowl plus the five-stop vertical Bg-main gradient ending at `50%`.
