...
- Runtime checkpoint: `d27a63d` (`fix(concept-v2): broaden large hero dot fade`).
...
- The current Large-mask contract sets its central curve at `286` (previously `370`) and uses a vertical-only `140px` Gaussian blur, so the middle is higher and the dot fade spans a much larger part of the field without horizontal haze.
