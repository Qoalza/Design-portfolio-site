...
- Ветка `codex/project-page-admin-surgical-fixes`: первая Git-группа готова к пользовательской проверке (`READY_FOR_USER_REVIEW`). Устранён белый экран «Страницы проекта»; pending-устройства галереи сохраняются отдельно от публичных групп. Загрузка первого изображения и коллизия одинаковых имён теперь сохраняют разные файлы без перезаписи, а ожидаемые ошибки формата/размера получают понятный текст. Отсутствующее подключение Figma показывается отдельной причиной; read-only живой импорт Frame в изолированный draft и его preview прошли без console errors. Исходные пользовательские черновики не менялись; push, PR, merge и deploy не выполнялись. Следующая отдельная группа — публичный финальный divider, только после read-only production/Figma эталона.
