...
- Настройки `Примечание` и `Интерактивный экран` находятся непосредственно внутри каждой секции и сохраняются независимо по `adminId`; правая панель больше не маскирует их как одну общую настройку. Rich-text редактор секции использует выровненный toolbar с Radix-иконками, единый focus-контур и чистую геометрию поля. Разные состояния пяти секций проверены после autosave и перезагрузки.
