...
- HEAD: `63716bb` (`fix(concept-v2): restore exact hero variant layouts`).
...
Hero uses the source compositions as separate structures: up to the authored Large canvas width of `2313px`, the runtime shows Small (left copy, right graph, caption above, no eyebrow); at or above it, it shows Large (centred copy with eyebrow, graph below, caption after). The accepted interactive graph/caption mechanics remain intact. The Experience Resume label is intentionally omitted by direct user decision; the icon-only link has `aria-label="Открыть резюме"`.
...
- `npm run check`: passed — lint, 62 tests, Vite production build.
