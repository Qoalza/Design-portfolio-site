...
- Admin follow-up переводит создание проекта на свободное название и системный уникальный slug, показывает все проблемы до preview/publish с навигацией к полям, заменяет browser-native окна внутренними Radix Dialog/AlertDialog, исправляет reorder главной и независимые настройки секций. Ручной необязательный logo принимает только пассивный квадратный SVG; production остаётся нетронутым, публикация по-прежнему dry-run.
