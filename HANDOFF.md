...
- Runtime checkpoint: `7d05de2` (`fix(concept-v2): deepen large hero dot wave`).
...
The user-requested Large central composition is raised a further `50px` (`translateY(-74.5px)`). This affects only the copy, controls and graph container; the independent dotted lower field stays at its existing coordinates. Large dots are `4×4px` on the same `16px` grid at `0 0`. Its fade is an in-CSS overlay of the exact Bg-main colour: a broad vertical gradient plus a central radial bowl makes the sides reveal higher than the centre, while the transition continues under the fact chip. The latest tuning shifts the fade lower and makes the wave clearer: the side field reaches transparency at `76%` of the `560px` field, while the radial centre remains to the field bottom (`100%`). This doubles the centre-versus-side depth separation from `13` to `24` percentage points while retaining broad intermediate stops. This deliberately replaces the Large external SVG filter, which did not produce a controllable visual result in the browser. Small retains the source SVG wave and `3×3px` dots.
...
- Large runtime inspection at `2400×1280`, DPR 1: the field is `560px` high, its grid remains `16px` at `0 0`, the obsolete Large SVG mask computes to `none`, and the CSS overlay resolves to a `48% × 100%` radial bowl plus the five-stop vertical Bg-main gradient ending at `76%`. The dots begin below the diagram's routes; the central bowl remains `24` percentage points longer than the sides, producing a visibly deeper wave.
