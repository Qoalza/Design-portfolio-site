...
- Runtime checkpoint: `7c9487c` (`fix(concept-v2): restore large hero dot wave`).
...
- Large runtime inspection at `2400×1280`, DPR 1: the field is `560px` high, its grid remains `16px` at `0 0`, the obsolete Large SVG mask computes to `none`, and the CSS overlay resolves to a `48% × 78%` radial bowl plus the five-stop vertical Bg-main gradient ending at `65%`. The dots begin below the diagram's routes; the central bowl remains longer than the sides, producing the visible wave.
