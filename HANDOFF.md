...
- Живой импорт пользовательского hero Frame выявил общий дефект `CENTER` constraints: процентное смещение ошибочно вычислялось относительно дочернего слоя. Расчёт исправлен одновременно в admin/public renderer; исходная геометрия Corvo Frame подтверждена в локальных admin и page preview. Импорт теперь сразу показывает spinner и текущий этап, повторный клик блокируется, input/action controls используют единые высоту и радиус, а обложка карточки расположена первой на вкладке `Карточка`.
