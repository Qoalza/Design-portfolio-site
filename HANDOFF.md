...
- Runtime checkpoint: `556c44b` (`fix(concept-v2): offset large hero composition`).
...
The user-requested Large central composition is raised a further `50px` (`translateY(-74.5px)`) and carries a `56px` bottom padding. This affects only the copy, controls and graph container; the independent dotted lower field stays at its existing coordinates. Large dots now use the same `3×3px` tile as Small on the same `16px` grid at `0 0`. Its fade is an in-CSS overlay of the exact Bg-main colour: a broad vertical gradient plus a central radial bowl makes the sides reveal higher than the centre, while the transition continues under the fact chip. The latest tuning deepens that bowl in the Large field: the side field reaches transparency at `52%` of the `560px` field, while the radial centre remains to the field bottom (`100%`). This creates a `48` percentage-point centre-versus-side depth separation and raises each side edge by another ≈`67px`, without changing Small. This deliberately replaces the Large external SVG filter, which did not produce a controllable visual result in the browser. Small retains the source SVG wave and `3×3px` dots.
...
- Large runtime inspection at `2400×1280`, DPR 1: the central container computes to `padding-bottom: 56px`; the independent field still begins at `720px` and is `560px` high. Its grid remains `16px` at `0 0`, both desktop variants use `3px` dots, the obsolete Large SVG mask computes to `none`, and the CSS overlay resolves to a `48% × 100%` radial bowl plus the five-stop vertical Bg-main gradient ending at `52%`. The dots begin below the diagram's routes; the central bowl remains `48` percentage points longer than the sides, producing a visibly deeper wave.
