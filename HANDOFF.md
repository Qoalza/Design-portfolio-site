...
- Runtime checkpoint: `e639ea9` (`fix(concept-v2): triple large hero fade span`).
...
- The current Large-mask contract sets its central curve at `286` (previously `370`) and uses a vertical-only `420px` Gaussian blur, so the middle is higher and the dot fade spans three times the previous range without horizontal haze.
