...
- Runtime checkpoint: `ec92254` (`fix(concept-v2): deepen large hero dot wave`).
...
- Large runtime inspection at `2400×1280`, DPR 1: the field is `560px` high, its grid remains `16px` at `0 0`, both desktop variants use `3px` dots, the obsolete Large SVG mask computes to `none`, and the CSS overlay resolves to a `48% × 100%` radial bowl plus the five-stop vertical Bg-main gradient ending at `52%`. The dots begin below the diagram's routes; the central bowl remains `48` percentage points longer than the sides, producing a visibly deeper wave.
