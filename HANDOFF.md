...
- В ветке `codex/figma-frame-admin-assets` добавлен структурированный `ProjectFrameComposition`: Figma Frame сохраняется как DOM/CSS-композиция с отдельными SVG/raster leaves, constraints, nested Auto Layout, root raster background и атомарным last-good snapshot. Production Figma не опрашивает. Cover/hero/interactive подключены без автоматической миграции legacy managed-композиций.
- Живой импорт пользовательского hero Frame выявил два общих дефекта renderer: `CENTER` constraints вычислялись относительно дочернего слоя, а вертикальные процентные `padding` в CSS ошибочно зависели от ширины контейнера. Расчёт исправлен одновременно в admin/public renderer через единицы корневого Frame; контрольная композиция повторно импортирована и визуально подтверждена в большом admin preview и в страничном preview. Cover-preview отдельно ограничен квадратом `320×320` и не сжимает композицию; input/action controls используют единые `32 px` и `6 px`. Outline-кнопки имеют белый base, серую обводку и различимые hover/pressed/focus/disabled состояния; destructive ghost-кнопки прозрачны в base и получают лёгкую заливку на hover. Повторяющиеся заголовки секций получают уникальные DOM id, поэтому duplicate-key overlay больше не возникает. Импорт сразу показывает spinner и текущий этап, повторный клик блокируется, а обложка карточки расположена первой на вкладке `Карточка`.
- Для поддерживаемых свойств structured importer сохраняет отдельные raster/SVG leaves, фон корневого и вложенного Frame, обводки, тени/blur, blend mode, clipping, radius, constraints и вложенный Auto Layout; обнаруженное неподдерживаемое свойство блокирует атомарный импорт с сохранением last-good snapshot вместо скрытого упрощения. Публичный список проектов больше не показывает подписи `Файл пока недоступен` или `У проекта нет отдельного файла`: состояние файла остаётся только внутри открытого проекта. Ошибки повреждённого импортированного Frame переводятся из внутренних contract paths в пользовательские названия ассета и конкретное действие восстановления.
...
3. Выполнить пользовательскую приёмку исправленного локального Frame preview из ветки `codex/figma-frame-admin-assets`. Только после отдельного разрешения exact SHA допускаются merge и production deploy.
