...
- Исправления страницы проекта и админки опубликованы: вкладка «Страница проекта» не падает, первое изображение устройства загружается при наличии Frame-каталога, а Desktop/Tablet/Mobile используют канонические Figma SVG-экспорты. Папки Frame не участвуют в дедупликации изображений; ожидаемые ошибки загрузки и Figma-импорта получают понятную причину без потери last-known-good данных.
- Публичная страница проекта сохраняет одну полноширинную границу перед Gallery, `40 px` вокруг terminal-интерактивного Frame и открывает изображения Gallery без CSS-обводки/радиуса. Изолированный browser preview подтвердил геометрию и чистую console до публикации.
...
- Односторонняя граница данных v2 активна: прежние sandbox drafts/assets/snapshots/jobs архивированы локально, рабочая база админки читает только канонические проекты и ассеты exact `origin/main`, а старый marker v1 не проходит live-readiness. Тестовые данные и тестовые состояния публикации не являются источником production-контента.
- Production deploy Goal выполнен атомарно 2026-08-31. Release-архивы исключают macOS xattrs/AppleDouble и локальные admin/runtime данные. Ограниченный SSH status и публичные `/`, `/projects`, `/projects/corvo` подтверждают один exact release SHA; все маршруты отвечают `200`.
...
- SSH-доступ ограничен командами status/upload/publish; предыдущий release сохраняется для атомарного rollback.
...
1. Production синхронизирован с `origin/main`; наблюдать штатными health/smoke-проверками без повторного deploy.
...
3. Новые изменения админки вести от чистого `main`; локальный `sandbox-archive` остаётся только восстановительным архивом и не публикуется.
