...
- Ветка `codex/project-page-admin-surgical-fixes`: первая Git-группа готова к пользовательской проверке (`READY_FOR_USER_REVIEW`). Устранён белый экран «Страницы проекта»; pending-устройства галереи сохраняются отдельно от публичных групп. Загрузка первого изображения и коллизия одинаковых имён теперь сохраняют разные файлы без перезаписи, а ожидаемые ошибки формата/размера получают понятный текст. Папки Frame больше не участвуют в дедупликации изображений, а Desktop/Tablet/Mobile используют проверенные Figma SVG-экспорты. Отсутствующее подключение Figma показывается отдельной причиной; read-only живой импорт Frame в изолированный draft и его preview прошли без console errors. Исходные пользовательские черновики не менялись; push, PR, merge и deploy не выполнялись. Следующая отдельная группа — публичные границы галереи и lightbox, только после явного пользовательского gate.
- Ветка `codex/project-page-admin-surgical-fixes`: публичная Git-группа исправляет три связанных условия страницы проекта. Последняя обычная секция при наличии Gallery больше не рисует собственный divider: остаётся один полноширинный `border-top` Gallery. У интерактивного Frame есть `40 px` после предыдущего контентного блока и `40 px` перед Gallery в terminal-секции. Открытое изображение Gallery не получает CSS-обводку/радиус; его размер выводится из фактической пропорции изображения и сохраняет viewport/DPR cap. Изолированный browser preview подтвердил `40 px`, один divider, совпадение rect media/image и пустую console; production, Figma и пользовательские drafts не менялись. После commit остановиться для пользовательской проверки; push, PR, merge и deploy исключены.
...
- Ветка `codex/project-page-admin-surgical-fixes` готовит повторную защитную миграцию baseline v2 перед следующим live publish: существующие drafts, preview overlays, draft-assets, snapshots и jobs архивируются локально, а чистая админка читает только проекты и ассеты exact `origin/main`. Старый marker v1 больше не проходит live-readiness. Миграция проверена на изолированном store; до merge/deploy реальные данные админки и production не менялись.
