...
2. **Перенести sandbox-черновики** — production остаётся baseline, а все присутствующие sandbox project-authoring значения накладываются в local live drafts. Sandbox побеждает, включая пустые значения, удаление, visibility, порядок и placement; jobs/previews/runtime state не переносятся. Bootstrap не публикует эти drafts и не меняет Git/canonical/production. Только последующее отдельное пользовательское нажатие «Опубликовать» может выпустить live drafts.

Второй путь требует отдельного явного выбора пользователя. Request привязан к exact production SHA; его изменение останавливает transition до archive. Launcher не делает manual review и не выбирает путь автоматически.
