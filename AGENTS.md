...
Второй путь требует отдельного явного выбора пользователя и безопасного three-way merge: `исходный production baseline → sandbox draft` накладывается на `текущий production baseline`. Неизменённые sandbox поля всегда берутся из current production; конфликт одной semantic unit останавливает перенос до archive и без частичного применения. Для старого sandbox без origin пользователь выбирает нужные units в локальной проверке, а выбор привязан к hash exact production-цели; её изменение также останавливает перенос до archive. Нельзя копировать sandbox draft/assets целиком или обходить этот механизм ручным переносом файлов.
