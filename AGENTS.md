...
Второй путь требует отдельного явного выбора пользователя и безопасного three-way merge: `исходный production baseline → sandbox draft` накладывается на `текущий production baseline`. Неизменённые sandbox поля всегда берутся из current production; конфликт одной semantic unit останавливает перенос до archive и без частичного применения. Для старого sandbox без origin Codex сначала показывает read-only review, получает выбор пользователя и только затем сохраняет локальный request; Admin UI не предлагает и не сохраняет этот выбор. Страница проекта переносится только одной целой unit: `detailAvailable` и hero вместе. Выбор привязан к hash exact production-цели; её изменение также останавливает перенос до archive. Нельзя копировать sandbox draft/assets целиком или обходить этот механизм ручным переносом файлов.
